#!/usr/bin/env bash
#
# rust-semver-parser/mayhem/test.sh — RUN steveklabnik/semver-parser's own test suite (`cargo test`)
# and emit a CTRF summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: the crate ships ~38 #[test]s that assert PARSED-VALUE EXACTNESS — version.rs
# (major/minor/patch, pre-release + build identifiers, ordering), lexer.rs (token sequences),
# parser.rs, range.rs and range_set.rs (operator/predicate decoding for ^, ~, >=, <, wildcard, etc.).
# These compare parser output against expected structured values, so a no-op / "exit(0)" /
# output-altering patch to the parser CANNOT pass. This script only RUNS the suite via `cargo test`;
# it never builds the fuzz target.
#
# Note: we run `cargo test` with the crate's NORMAL flags (default resolution of the installed
# toolchain) — no sanitizer RUSTFLAGS — to keep the oracle honest and fast. The genpest.rs
# "generated_code_is_fresh" test fast-paths to a no-op when src/generated.rs is current (it only
# invokes rustfmt when the grammar changed), so it does not slow the run on an unmodified tree.
set -uo pipefail
...
echo "=== running cargo test (semver-parser unit + integration tests) ==="
